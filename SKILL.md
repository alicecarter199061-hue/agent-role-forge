...
5. **智能调度执行模式**：审问第 0 步先判断 **single（单角色）还是 team（多角色团队）**，自动路由到不同流程——一个角色走标准 6 步；一个团队走"全局审问 + 逐角色 DIY + 角色分工表"。

## How It Works（6 步工作流）

### Step 1 — 审问需求（grill）

**第 0 步（智能调度）：先判断执行模式**——按 `references/grill-questions.md` 的「角色结构判断」确认 single 还是 team：

- **single**：标准流程（下面 Step 2~6 走一轮，产出 1 份 prompt）。
- **team**：切换团队流程——先全局审问（团队目标/平台/共享权限/协作关系），再逐角色审问（定位/触发/专属权限/风格），需求卡里列出角色清单。团队流程见下方「team 模式分支」。

single 模式按必问 5 项审问，直到以下明确：
...
审问原则：不问废话，已明确的项跳过；用户说"直接干/轻量"时降级为 1 轮快速确认（只问目标 + 边界）。触发词命中"小队/团队/几个角色/分工/协作"时自动切 team。

**Step 1 完成判据（single）**：产出「角色需求卡」且必问 5 项（目标/触发场景/平台模型/权限/风格）均有明确答案；有缺项则继续审问。
**Step 1 完成判据（team）**：产出「团队需求卡」含团队目标 + 角色清单（每个角色有定位/触发/权限/风格）+ 协作关系（指挥/串并行/汇总）。

---

### team 模式分支（Step 2~6 按团队循环）

1. **团队取材**：Step 2 索引重点看**多角色协作机制**——CrewAI（角色定义 + 分工）、MetaGPT（SOP 标准化交付）、AutoGen（角色间消息交互）、D1 设计模式（指挥/执行分离、编排方案选型）。
2. **逐个角色取材 + DIY**：对角色清单中每个角色，循环执行 Step 2（匹配索引）→ Step 3（按需检索）→ Step 4（取材该角色的机制）→ Step 5（按 output-template 产出该角色的 prompt，覆盖 6 节 + 来源标注）。
3. **团队结构设计**：若用户没给角色清单，按任务拆解建议（参考 MetaGPT SOP 各环节），让用户确认后再逐角色 DIY。
4. **Step 6 展示**：输出**角色分工表**（角色名/定位/协作关系）+ 每份 prompt + 参考来源清单，等用户确认后逐个创建。
...
- **team 模式**额外参考 → CrewAI（角色定义+分工）、MetaGPT（SOP 标准化）、AutoGen（角色消息交互）、D1 设计模式（指挥/执行分离、编排方案选型）

选中 1~3 个高度相关条目，记录其"可借鉴机制"。team 模式对每个角色分别匹配其任务类型。
...
**Step 5 完成判据（single）**：产出 prompt 覆盖 output-template 的全部 6 节（Identity/Responsibilities/Boundaries/Tools/Style/Constraints），且每条借鉴机制带来源标注。
**Step 5 完成判据（team）**：角色清单中**每个角色**都产出覆盖 6 节的 prompt + 来源标注；无遗漏角色。

### Step 6 — 展示 + 确认

- 输出（single）：完整通用 prompt 文本 + 参考来源清单 + 可借鉴机制摘要
- 输出（team）：**角色分工表**（角色名/定位/协作关系）+ 每份 prompt + 参考来源清单
...
**Step 6 完成判据（single）**：已展示 prompt + 来源 + 平台接入一步，且未执行任何创建动作（除非用户显式确认）。
**Step 6 完成判据（team）**：已展示分工表 + 每份 prompt + 来源，且未执行任何创建动作（除非用户显式确认）。
